...
namespace Core.Azure
{
    ...
    public class MigrationTableStorage : TypedTableStorage<MigrationInfo>
    {
        public MigrationTableStorage(CloudTableClient tableClient, AzureContextOptions options, string name)
            ...
            while (iteration > 0)
            {
                ...
                if (log == null)
                {
                    ...
                    try
                    {
                        ...
                        return new MigrationResult { Status = MigrationStatus.Migrating, Info = log };
                    }
                ...
                else
                {
                    if (log.Status == MigrationStatus.Migrating)
                        ...
                        return new MigrationResult { Status = MigrationStatus.Done, Info = log };
                    else if (log.Status == MigrationStatus.Error)
                        return new MigrationResult { Status = log.Status, Info = log };
                    else
                        return new MigrationResult { Status = MigrationStatus.InternalError, Info = log, Error = $"Invalid status: {log.Status}" };
                }
            }

            return new MigrationResult { Status = MigrationStatus.InternalError, Error = "Retry count exceeds limit" };
        }
        ...
        public async Task MigrationDone(ILogger logger, MigrationResult result)
        {
        ...
        public async Task MigrationFailed(ILogger logger, MigrationResult result, string error)
        {
            Trace.Assert(result.Status == MigrationStatus.Migrating);
