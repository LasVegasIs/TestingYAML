...
namespace Core.Azure
{
    ...
    {
        public static CloudTableClient CreateClient(CloudStorageAccount saConnection)
        {
            var tableClient = saConnection.CreateCloudTableClient().ThrowIfNull(() => new Exception($"Cannot create table client"));
    ...
    public class TableStorage
    {
        ...
        public TableStorage(CloudTableClient tableClient, string tableName)
        {
            tableName_ = tableName;
            Table = tableClient.GetTableReference(tableName_);
        }

        public TableStorage(CloudTableClient tableClient, string tableName, bool ensureExists)
        {
            tableName_ = tableName;
    ...
    public class TypedTableStorage<Entry> : TableStorage
        where Entry : TableEntity, new()
    {
        public TypedTableStorage(CloudTableClient tableClient, AzureContextOptions options)
            : base(tableClient, options.GetTableName(typeof(Entry).Name))
        {
        }

        public TypedTableStorage(CloudTableClient tableClient, AzureContextOptions options, string name)
            : base(tableClient, options.GetTableName(name))
        {
        }

        public TypedTableStorage(CloudTableClient tableClient, AzureContextOptions options, string name, bool createOnDemand)
