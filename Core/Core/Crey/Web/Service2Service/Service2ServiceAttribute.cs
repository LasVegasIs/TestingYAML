    ...
    public class ServerToServerAttribute : Attribute
    {
        ...
        public ServerToServerAttribute(string policy)
        {
            switch (policy)
            {
                ...
                default:
                    throw new ServerErrorException($"Invalid policy: {policy}");
            }
        }
        ...
        public static void Validate(HttpContext httpContext)
        {
            ...
            if (attrib != null)
            {
                ...
                if (HttpUtility.UrlEncode(requiredKey) != requiredKey)
                    throw new ServerErrorException($"Key containes invalid characters");

                if (headerKey != requiredKey)
                    throw new AccessDeniedException($"Invalid api key {attrib.HeaderName}");
            }
        }
    }
}
