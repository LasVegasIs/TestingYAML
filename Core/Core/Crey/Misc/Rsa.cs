...
namespace Crey.Misc
{
    public static class Rsa
    {
        ...
        private static readonly byte[] _publicExponentKey = new byte[]
        {
            0x11,
        };
        ...
        private static readonly BigInteger _publicExponent = new BigInteger(_publicExponentKey);
        ...
        // 
        //  IMPORTANT:  it will trim the 0x00 bytes from the beginning of the input array
        //
        //  input:              decrypted:
        ...
        {
            if (input[0] == 0x00) return null;
            if (input.Length > _modulusKey.Length) return null;

            var encryptedData = input.Reverse().ConcatSingle((byte)0).ToArray();

            var output = BigInteger.ModPow(new BigInteger(encryptedData), _publicExponent, _modulus).ToByteArray().Reverse().ToArray();
